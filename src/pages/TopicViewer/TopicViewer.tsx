...
function TopicViewer() {
  ...
  const nodeResetRef = useRef<(() => void) | null>(null)
  const nodeRunRef = useRef<(() => void) | null>(null)
  ...
  function handleBarButton() {
    if (['Bubble Sort', 'Selection Sort', 'Quick Sort'].includes(activeTopic)) {
      setIsArray(true)
    } else {
      setIsArray(false)
    }
        ...
        <div className={styles.visualPanel}>
            ...
            {gridMode
              ...
              : isGraphAlgo
                ? <NodeVisualiser
                    ...
                    onStepsGenerated={setTotalSteps}
                    activeTopic={activeTopic}
                    onNodeReady={(reset, run) => {
                      nodeResetRef.current = reset
                      nodeRunRef.current = run
                    }}
                  />
                : Visualiser
                  ? <Visualiser
          ...
          <div className={styles.controls}>
            {gridMode ? (
              ...
              <>
                <button className={styles.controlBtn} onClick={() => {
                  if (isGraphAlgo) nodeResetRef.current?.()
                  setCurrentStep(0)
                  setTotalSteps(0)
                }}>Reset</button>
                <button className={styles.controlBtn} onClick={() => setCurrentStep(s => Math.max(0, s - 1))}>Back</button>
                <button className={styles.controlBtn} onClick={() => {
                  if (isGraphAlgo) nodeRunRef.current?.()
                  setIsPlaying(p => !p)
                }}>
                  {isPlaying ? 'Pause' : 'Play'}
                </button>
